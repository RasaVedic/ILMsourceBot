...
# --- Direct command shortcuts (NEW) ---

async def cmd_add_topic(update, context):
    await update.message.reply_text("Naya topic ka naam bhejein:")
    return ADD_TOPIC

async def cmd_add_content(update, context):
    from utils.fileops import get_topics
    topics = get_topics()
    if not topics:
        await update.message.reply_text("Koi topic nahi mila. Pehle topic add karein.")
        return ConversationHandler.END
    keyboard = [[InlineKeyboardButton(t, callback_data=f'addcontenttopic_{t}')] for t in topics]
    await update.message.reply_text("Kaunse topic me content add karna hai?", reply_markup=InlineKeyboardMarkup(keyboard))
    return ADD_CONTENT_TOPIC

async def cmd_add_meme(update, context):
